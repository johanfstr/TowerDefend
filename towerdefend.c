...
int **initChemin(int *nbcase, int *x, int *y){
    ...
    while (d < 20){
        ...
        if (alea == 0 && (ydepart - nbdistance >= 1)){
        ...
        else if (alea == 2 && (xdepart + nbdistance) < LARGEURJEU && brider == 0){
...
TListePlayer quiEstAPortee(TplateauJeu jeu, Tunite *UniteAttaquante)
{
    ...
    for (int i = UniteAttaquante->posX - portee; i <= UniteAttaquante->posX + portee; i++) { // parcours le plateau pour detecter ce qui est a portee
